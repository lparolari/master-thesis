...
the image, $Q_S = \{ \bm{q}_j \}^m_{j=1}$ the set of noun phrases
built on the sentence $S$ that describes $\bm{I}$ and $Q'_S = \{
\bm{q}_h \}^{m'}_{h=1}$ the set of noun phrases built on $S'$ that do
not describe $\bm{I}$, we define $\calL$ as:
\begin{equation}
...
$k = 100$ proposals. (We studied how $k$ can affect results in
